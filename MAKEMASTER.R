...
{
G38= read.table(file="./homo_sapiens.GRCh38_trimmed.bed", sep="\t",quote="",comment.char="")
...
########################Referring to the "IMP_COLUMNS_for_each_DB2.R" and finding those databases that has useful details in it.
...
{ 
  ...
  write.table(finally,file=paste0("./masterfiles_new/",i,"_masterfile.bed"),quote=FALSE,row.names=FALSE,sep="\t")
